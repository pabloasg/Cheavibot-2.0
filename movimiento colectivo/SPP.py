...
'''
este archivo contiene la regla de alineacion y cohesion, sin embargo no esta totalmente desarrollado, hay un error en la regla de cohesion, en donde se considera incluso al bot que esta en el medio
siendo que solo se deben considerar a los vecinos
'''
...
NEGRO= (255,255,255)#(0,0,0)
BLANCO=(0,0,0)#(255,255,255)
...
for p in range (0, paso):
    ...
    for h in range(0,len(B)):  
        ...
        pygame.draw.circle(pantalla, (0,100,0), (int(cor[0]), int(cor[1])), 6*r, 1)
        ...
        if a11==True and b11==False:
            ...
            pygame.draw.circle(pantalla, (255,0,0), (int(xc1), int(yc1)), int(r*0.2), 0)
            pygame.draw.line(pantalla, (255,0,0), A[h], (xc1, yc1))
            pygame.draw.line(pantalla, (255,0,0), (50,900), (xc1+50-A[h][0], yc1+900-A[h][1]),1)
        ...
        if h==len(B)-1: #en esta aplicacion en particular esta linea hace que el codigo se aplique al principio de cada paso
            ...
            for k in range (0, len(B)):
                ...
                if (k in al) ==True:
                    ...
                    p1=(B[k].getvel()*math.cos(B[k].getvel()), B[k].getvel()*math.sin(B[k].getvel()))
                    p2=B[k].getpos()
                    p3=xc1, yc1
        ...
        print(contadorv,'numero operaciones vecinos  ',len(vecinos), 'numero de vecinos' )
