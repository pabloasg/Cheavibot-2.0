...
'''
este archivo junta las 3 reglas de movimiento colectivo, la simulacion ocurre paso a paso y se muestra que regla sigue cada bot
este archivo se llama FT (final test), es un archivo de prueba para debugear las 3 reglas, por esta razon se ejecuta paso a paso
en donde cada paso es lo que sucede con un bot individual, al final de recorrer todos los bots se mueven todos los bots
'''
...
for p in range (0, paso):
    ...
    for h in range(0,len(B)):  
        ...
        pygame.draw.circle(pantalla, (0,255,0), (int(cor[0]), int(cor[1])), 8*r, 1)
        pygame.draw.circle(pantalla, (0,0,255), (int(cor[0]), int(cor[1])), 6*r, 1)
        pygame.draw.circle(pantalla, (255,0,0), (int(cor[0]), int(cor[1])), 3*r, 1)
        ...
        name=('paso N'+str(p)+'.jpeg')
        pygame.image.save(pantalla, name)
        
        pygame.display.flip()
        my_clock.tick(100) #FPS de la ventana y tambien de la actualizacion de los datos
        print(regla)
pygame.quit()